...
public:
	...
	void test_rearrange_examples()
	{
		auto test1 = [this](auto x)
		{
			...
			CHECK(dump(y.sizes()), dump({ 10, 30, 40, 20 }));
		};
		...
		auto test8 = [this](auto x)
		{
			...
			CHECK(dump(y.sizes()), dump({ 10, 20, 30 / 2, 40 / 2 }));
		};
	
		auto test9 = [this](auto x)
		{
			auto y = reduce(x, "b c h w -> b c () ()", "max");
			CHECK(dump(y.sizes()), dump({ 10, 20, 1, 1 }));
			y = rearrange(y, "b c () () -> c b");
			CHECK(dump(y.sizes()), dump({ 20, 10 }));
		};
		...
		auto test11 = [this](auto x)
		{
			...
			CHECK(dump(y.sizes()), dump({ 30, 10 * 40, 20 }));
		};
