...
public:
	...
	void test_rearrange_examples()
	{
		...
		auto test6 = [this](auto x)
		{
			...
			t = t.index({ Slice(), Slice(None, None, 2) });
			TESTS(dump(t), dump({ 10 * 30 * 40, 10 }));

			auto y = rearrange(t, "(b h w) c2 -> b c2 h w", parse_shape(x, "b _ h w"));
			TESTS(dump(y), dump({ 10, 10, 30, 40 }));
		};
		
		auto test7 = [this](auto x)
		{
			auto [y1, y2] = split(rearrange(x, "b (c g) h w -> g b c h w", axis("g", 2)));
			TESTS(dump(y1), dump({ 10, 10, 30, 40 }));
			TESTS(dump(y2), dump({ 10, 10, 30, 40 }));
		};
