...
public:
    ...
    void test_reduce()
    {
        ...
        }
        {
            auto x = random({ 10, 20, 30, 40 });
            {
                ...
                TESTS(dump(y1.sizes()), dump({ 10, 20, 15, 20 }));
            ...
            }
            {
                ...
                TESTS(dump(y.sizes()), dump({ 10, 20, 3, 4 }));
            }
            {
                // Global average pooling
                auto y = reduce(x, "b c h w -> b c", "mean");
                TESTS(dump(y.sizes()), dump({ 10, 20 }));
            }
            {
                // Subtracting mean over batch for each channel
                auto y = x - reduce(x, "b c h w -> () c () ()", "mean");
                TESTS(dump(y.sizes()), dump({ 10, 20, 30, 40 }));
            }
            {
    ...
    void test_repeat()
    {
        ...
        }
        {
            ...
            TESTS(dump(result.sizes()), dump({ 30, 40 }));
        }
    }
